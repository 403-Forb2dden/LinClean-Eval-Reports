...
대표 지표는 **전체 악성 기준 탐지율 91.23%**로 설명합니다.

- 계산식: `탐지 성공 104건 / 전체 악성 기대값 114건`
- 의미: OpenPhish 최신 URL과 자체 악성 샘플을 모두 포함해, 실제 악성으로 준비한 전체 테스트셋 중 몇 건을 탐지 판정까지 연결했는지 보여줍니다.
- 보수적으로 보는 이유: 사라진 페이지나 400번대 접근 실패처럼 `PAGE_UNAVAILABLE`로 조기 종료된 악성 URL도 전체 악성 기준 분모에 포함합니다.
- 함께 설명할 점: 미탐(FN)은 0건이고, 탐지하지 못한 10건은 악성으로 오판한 것이 아니라 페이지 미발견/접근 불가로 verdict 없이 종료된 케이스입니다.

발표 문장 예시는 다음과 같습니다.

> 전체 악성 URL 114건 중 104건을 탐지해 전체 기준 탐지율은 91.23%입니다. 나머지 10건은 사라졌거나 접근 불가능한 페이지로 분리되어 verdict 없이 조기 종료되었고, 판정 가능한 악성 URL 기준 Recall은 100%입니다.

| 구분 | 값 | 설명 |
|---|---:|---|
| 전체 악성 기준 탐지율 | 91.23% | 발표 대표 지표. 전체 악성 114건 중 104건 탐지 |
